...
class EczaneScraping:
    ...
    async def scrape_il(self, il_adi):
        ...
        tarihler = [self.prepare_data(il_adi, gun_sira=i) for i in range(3)]  # İlgili il için 3 farklı tarih oluşturulur
        print("Traihler = ",tarihler)
        sonuc=[]
        for tarih in tarihler:
            data = tarih  # İl için özel JSON verisini alın ve formatını düzenleyin
            print("DATA = ",data)
            ...
            if url:
                print(data)
                response = requests.post(url, data=data,headers=headers)  # Veriyi POST olarak gönderiyoruz
                ...
                try:
                    ...
                    sonuc.append(fonksiyon(eczane_raw_data))
    ...
    def prepare_data(self, il_adi,gun_sira=0):
        ...
        for key, value in yeni_json.items():
            if key.startswith("tarih"):
                ...
                today = datetime.now() + timedelta(days=gun_sira)
                tarih = today.strftime(tarih_format)
                print(tarih)
    ...
    def izmir(self,scrabed_data):
        ...
        for i in scrabed_data:
            try:
                ...
                print(konum)
                print(" ------------------ ")
                denemeler={"eczane_adi":ecza_adi.strip(),"telefon": telefon,"adres":adres.strip(), "ilce":ilce.strip(), "konum":konum,"tarih":tarih.strip()}
                print(denemeler)
                pharma_data.append(denemeler)
    ...
    def edirne(self,scrabed_data):
        ...
        for i in scrabed_data:
            ...
            print(ilce)
            print("------------------------")
            pharma_data.append({"eczane_adi":ecza_adi.strip(),"telefon": telefon,"adres":adresler, "ilce":ilce.strip(), "konum":konum.strip(),"tarih":tarih.strip()})
        return pharma_data
    ...
    def denizli(self,scrabed_data,ilce):
        ...
        for i in scrabed_data:
            ...
            #ilce=ilce.upper()
            print(telefon)
            print(eczane_adi.strip())
            print(adres)
            #print(ilce.strip())
            print(konum)
            print(" ")
            #TODO: ILCE VERISI EKLENMELI
            #pharma_data.append({"eczane_adi":eczane_adi.strip(),"telefon": telefon,"adres":adres, "ilce":ilce.strip(), "konum":konum.strip(),"tarih":tarih.strip()})
        return pharma_data